...
- scenes: 分镜列表
  ...
  - lines: 台词时间轴列表（可为空）
    ...
    - text: 台词内容（🔴 死规定：字数 ≤ (end_sec - start_sec) × 3，即每秒最多3个字）
  - text_overlays: 画面文字列表（选填），所有需要在画面中展示的文字写在这里，不要写在 visual_desc 里
    - text: 文字内容
    - x: 水平位置百分比（0=最左，50=居中，100=最右，默认50）
    - y: 垂直位置百分比（0=顶部，50=居中，100=底部，默认50）
    - font_size: 字号（默认36）
    - color: 颜色十六进制（如"#FFFFFF"白色，默认白色）
    - align: 对齐方式（"center"/"left"/"right"，默认"center"）
