...
async def studio_burn_subtitles(body: dict, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    try:
        ...
        out_path = os.path.join(tmpdir, out_name)
        result = subprocess.run(
            ["ffmpeg", "-i", vid_path, "-vf", f"subtitles={srt_path}:force_style='FontName=Microsoft YaHei\,FontSize=18\,PrimaryColour=&H00FFFFFF\,OutlineColour=&H00000000\,BorderStyle=1\,Outline=1'",
             "-c:a", "copy", "-y", out_path],
            capture_output=True, text=True, timeout=120,
        )
        if result.returncode != 0:
