            ...
            try:
                ...
                if wf_cfg:
                    ...
                    if api_key:
                        ...
                        prompt = f"""你是一个ASR纠错助手。你的任务是修正语音识别产生的错别字、同音字错误和标点符号。仅修正明显的识别错误，不要改写句子结构，不要替换同义词，不要根据剧情修改内容。修正原则：1) 只改错别字和同音字 2) 只加标点符号 3) 保持原文措辞和句式不变 4) 不确定的地方保持原样 5) 不要参考任何外部信息来修改内容"""
