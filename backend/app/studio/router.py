            ...
            try:
                ...
                if wf_cfg:
                    ...
                    if api_key:
                        ...
                        prompt = f"""你是一个ASR纠错助手。修正语音识别中的错别字和同音字。"""

                        if script_context:
                            prompt += f"""

参考剧本台词（仅用于纠正同音字/错别字，用于判断哪些词是正确写法）：
{script_context}
"""
                        prompt += f"""

【严格规则 - 必须遵守】：
1. 【禁止加台词】原始ASR文本中没有的内容，严禁添加。如果剧本有某句台词但ASR没说，不能补上去
2. 【禁止加字】素材（原始ASR文本）中没出现的字，绝对不能自己加进去
3. 【旁白处理】带[旁白]标记的行是画外音（旁白），不是视频中人物在说话。保持旁白标记不变，只改错别字
4. 【错别字修正】只改明显错别字和同音字（如：自率→自律，隔养→隔氧）
5. 【标点符号】只加必要的标点符号，不改动措辞和句式
6. 【剧本参考】剧本中的正确词汇优先用于修正对应位置的错别字
7. 【不确定则保留】不确定的地方保持原样，宁可不改也不要乱改
...
只输出修正后的文本，每行对应一行："""
                        payload = {
                            "model": model,
                            "messages": [
                                {"role": "system", "content": "你是一个字幕校对助手。严格遵守：1.只改错别字同音字 2.不添加ASR原文没有的内容 3.旁白标记保持不动 4.不确定的保持原样。只输出修正后的文本，不要额外内容。"},
